...
export default function ShareRoomModal({ isOpen, onClose, roomId }: ShareRoomModalProps) {
  ...
  // 生成分享链接
  const shareUrl = `${typeof window !== 'undefined' ? window.location.origin : ''}/gomoku_project?room=${roomId}`;
  const shareText = `加入我的五子棋对战房间 #${roomId}`;
  ...
  const handleDownloadQR = () => {
    if (qrCodeUrl) {
      ...
      link.download = `gomoku-room-${roomId}-qr.png`;
      link.href = qrCodeUrl;
      link.click();
    }
  };
